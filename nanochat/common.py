...
def get_eval_bundle_dir():
    """
    Returns the eval bundle directory, which holds the data for both CORE and the
    chat evaluations. It is prepared out of band (see runs/prepare.sh), so a missing
    directory is an error rather than something to download or fall back from.
    """
    eval_bundle_dir = os.path.join(get_base_dir(), "eval_bundle")
    if not os.path.isdir(eval_bundle_dir):
        raise FileNotFoundError(
            f"Eval bundle not found at {eval_bundle_dir}. "
            "Prepare it first (runs/prepare.sh)."
        )
