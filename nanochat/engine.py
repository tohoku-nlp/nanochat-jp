...
class Engine:
    ...
    def generate_batch(self, tokens, num_samples=1, include_terminal=False, **kwargs):
        """
        Non-streaming batch generation that just returns the final token sequences.
        Returns token sequences and sampling masks as lists of lists.

        Terminal tokens (assistant_end, bos) are omitted by default for backwards
        compatibility with inference callers. Set include_terminal=True for policy
        gradient training, where the sampled terminal action must remain supervised.
        """
