...
_ANSWER_LABEL_PATTERN = re.compile(r"(?<![A-Za-z0-9])([A-D])(?![A-Za-z0-9])")


def extract_last_choice_label(response):
    """Extract the last standalone A-through-D label from a response."""
    normalized_response = unicodedata.normalize("NFKC", response)
    matches = _ANSWER_LABEL_PATTERN.findall(normalized_response)
    return matches[-1] if matches else None


def deterministic_fewshot_sample(seed_material, candidates, num_fewshot, salt=""):
    """Sample target-dependent few-shot rows with a stable SHA-256 seed."""
    if num_fewshot < 0:
        raise ValueError("num_fewshot must be non-negative")
    if len(candidates) < num_fewshot:
        raise ValueError(
            f"Need {num_fewshot} few-shot candidates, but only {len(candidates)} exist"
        )
    ...
    seed_bytes = hashlib.sha256(
        (salt + "\0" + canonical_json(seed_material)).encode("utf-8")
    ).digest()
    rng = random.Random(int.from_bytes(seed_bytes, byteorder="big"))
    return rng.sample(list(candidates), num_fewshot)
...
def render_mc(question, letters, choices):
    """Render choices in the shared ``A: choice text`` format."""
