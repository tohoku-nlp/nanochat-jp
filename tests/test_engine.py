...
class ScriptedModel(MockModel):
    """Mock model that greedily emits a fixed token sequence."""

    def __init__(self, token_sequence, vocab_size=262):
    ...
    def forward(self, ids, kv_cache=None):
        ...
        if kv_cache is not None:
            kv_cache.advance(T)
        token_idx = min(self.forward_calls, len(self.token_sequence) - 1)
        next_token = self.token_sequence[token_idx]
...
def test_generate_batch_terminal_token_is_optional():
    """Inference omits terminal tokens while RL callers can retain them and their masks."""
    prompt = [261, 72, 105]

    default_engine = Engine(ScriptedModel([260]), ByteTokenizer())
    default_results, default_masks = default_engine.generate_batch(
        prompt,
        temperature=0.0,
        max_tokens=1,
    )
    assert default_results == [prompt]
    assert default_masks == [[0] * len(prompt)]

    training_engine = Engine(ScriptedModel([260]), ByteTokenizer())
    training_results, training_masks = training_engine.generate_batch(
        prompt,
        temperature=0.0,
        max_tokens=1,
        include_terminal=True,
    )
    assert training_results == [prompt + [260]]
    assert training_masks == [[0] * len(prompt) + [1]]


def test_generate_batch_retains_terminal_at_max_token_boundary():
    """A terminal sampled in the final allowed position remains distinguishable from truncation."""
    prompt = [261, 72, 105]
    engine = Engine(ScriptedModel([65, 260]), ByteTokenizer())
    ...
    assert results == [prompt + [65, 260]]
    assert masks == [[0] * len(prompt) + [1, 1]]
